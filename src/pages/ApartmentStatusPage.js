...
export default function ApartmentStatusPage() {
    ...
    const handleTurnOn = async () => {
        try {
            const res = await fetch(
                ...
                { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'apartment_on' }) }
            );
            if (!res.ok) throw new Error();
            await res.json();
            fetchStatus();
    ...
    const handleTurnOff = async () => {
        try {
            const res = await fetch(
                `https://gtw06or8tl.execute-api.eu-north-1.amazonaws.com/test/api/v1/apartments/${aptId}/statuses`,
                { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'apartment_off' }) }
            );
            if (!res.ok) throw new Error();
            await res.json();
            fetchStatus();
